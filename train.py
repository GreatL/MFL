...
def train(args):
    ...
    if args.save:
        if not args.save_dir:
            ...
            models_dir = os.path.join('logs', 'nc', date)
    ...
    args.n_nodes, args.feature_dim = data['text_features'].shape
    args.feat_dim = args.dim
    no_text = 0
    no_image = 1
    only_adj = 0
    if no_text == 1:
        data['text_features'] = torch.zeros_like(data['text_features'])
    if no_image == 1:
        data['image_features'] = torch.zeros_like(data['image_features'])
    Model = NCModel
    ...
    if args.optimizer == 'radam':
        ...
        optimizer = optim.Adam(params=optimizer_grouped_parameters,
                                  lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        train_metrics, _ = model.compute_metrics(embeddings, data, 'train', args.lamda)
        ...
        with torch.no_grad():
            if (epoch + 1) % args.eval_freq == 0:
                model.eval()
                embeddings = model.encode(data['text_features'], data['image_features'], data['adj_train_norm'])
                val_metrics, _ = model.compute_metrics(embeddings, data, 'val', args.lamda)
                if (epoch + 1) % args.log_freq == 0:
                    logging.info(" ".join([
                        'Epoch: {:04d}'.format(epoch + 1),
                ...
                if model.has_improved(best_val_metrics, val_metrics):
                    best_test_metrics, classification_report = model.compute_metrics(
                        embeddings, data, 'test', args.lamda)
    ...
    if not best_test_metrics:
        ...
        best_test_metrics, classification_report = model.compute_metrics(best_emb, data, 'test', args.lamda)
    logging.info(" ".join(
        ["Val set results:",
